...
IMPORT_MAP(hudm);
IMPORT_MAP(mapmission17);

const UINT8 coll_m17_tiles[] = {3, 17, 18, 19, 21, 22, 23, 28, 29, 36, 44, 45, 46, 47, 49, 51, 52, 54, 56, 57, 58, 59, 61, 62, 63, 65, 66, 67, 68, 69, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 82, 83, 84, 85, 86, 87, 89, 90, 91, 92, 93, 94, 95, 118, 119, 120, 121, 0};

const UINT8 coll_m17_surface[] = {0u, 0};
...
void START(){
    ...
    //COMMONS & START
        InitScroll(BANK(mapmission17), &mapmission17, coll_m17_tiles, coll_m17_surface);
		INIT_HUD(hudm);
		SetWindowY(104);
        spawn_items();
        start_common();
